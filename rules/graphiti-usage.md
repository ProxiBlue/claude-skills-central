...
1. **Before EVERY in-conversation `add_memory`:** classify scope and confirm with the user, one line, then wait:
   ...
   Project id = `$GRAPHITI_GROUP_ID` if set, else `$DDEV_PROJECT`, else `basename $(git rev-parse --show-toplevel)`. Never `default`, `main`, or session ids. `GRAPHITI_GROUP_ID` exists for git-worktree-per-ticket setups (pps) where `$DDEV_PROJECT` is ticket-suffixed (e.g. `pvcpipesupplies-479`) but the knowledge graph must stay unified under the one real project name (`pvcpipesupplies`) — set explicitly per worktree, unset everywhere else, so this is a no-op for every non-worktree project. (Plugin's non-interactive consolidation hooks are exempt — they resolve scope deterministically.)
